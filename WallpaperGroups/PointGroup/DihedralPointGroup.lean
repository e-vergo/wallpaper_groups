...
-/
import WallpaperGroups.PointGroup.CyclicPointGroup

/-!
# Dihedral Point Groups

This file defines the dihedral point groups Dₙ as subgroups of O(2) and establishes
their isomorphism with Mathlib's DihedralGroup.

## Main results

* `DihedralPointGroup.card` - |Dₙ| = 2n
* `DihedralPointGroup.equivDihedralGroup` - Dₙ ≃* DihedralGroup n
* `DihedralPointGroup.semidirectProduct` - Dₙ ≅ Cₙ ⋊ C₂
* `DihedralPointGroup.two_isKleinFour` - D₂ ≃* Z₂ × Z₂
* `CyclicPointGroup.eq_dihedral_inter_SO2` - Cₙ = Dₙ ∩ SO(2)
-/
...
private lemma reflectionMatrix'_angleOf_r_mul_sr (n : ℕ) [NeZero n] (i j : ZMod n) :
  ...
  · -- Wrap: (j - i).val = j.val + n - i.val
    ...
    -- RHS = -(j.val + n - i.val) * (2π/n)
    --     = (i.val - j.val - n) * (2π/n) = (i.val - j.val) * (2π/n) - 2π
...
lemma dihedralToO2_injective (n : ℕ) [NeZero n] : Function.Injective (dihedralToO2 n) := by
  ...
  | r i =>
    ...
    | r j =>
      ...
      simp only [rotationMatrix, Matrix.of_apply, Matrix.cons_val_zero,
        Matrix.cons_val_one] at h00 h10
  ...
  | sr i =>
    ...
    | sr j =>
      ...
      simp only [reflectionMatrix, Matrix.of_apply, Matrix.cons_val_zero,
        Matrix.cons_val_one] at h00 h10
...
lemma CyclicPointGroup.normal_in_dihedral (n : ℕ) [NeZero n] :
  ...
  have key : ((CyclicPointGroup n).subgroupOf (DihedralPointGroup n)).index *
      Nat.card (CyclicPointGroup n) = 2 * n := h
  rw [hcard_C] at key
  exact Nat.eq_of_mul_eq_mul_right hn_pos key

-- =============================================================================
-- Semidirect product structure: Dₙ ≅ Cₙ ⋊ C₂
-- =============================================================================

-- Helper lemma for ZMod 2
private lemma zmod2_eq_zero_or_one (a : ZMod 2) : a = 0 ∨ a = 1 := by fin_cases a <;> simp

/-- The inversion automorphism on Multiplicative (ZMod n). -/
private noncomputable def invAutZMod (n : ℕ) : MulAut (Multiplicative (ZMod n)) where
  toFun := fun x => Multiplicative.ofAdd (-(Multiplicative.toAdd x))
  invFun := fun x => Multiplicative.ofAdd (-(Multiplicative.toAdd x))
  left_inv := fun x => by simp
  right_inv := fun x => by simp
  map_mul' := fun x y => by rw [toAdd_mul, neg_add, ofAdd_add]

@[simp] private lemma invAutZMod_apply (n : ℕ) (x : Multiplicative (ZMod n)) :
    invAutZMod n x = Multiplicative.ofAdd (-(Multiplicative.toAdd x)) := rfl

/-- The action of ZMod 2 on ZMod n by negation: 0 acts trivially, 1 acts by inversion. -/
private noncomputable def negActionZMod (n : ℕ) :
    Multiplicative (ZMod 2) →* MulAut (Multiplicative (ZMod n)) where
  toFun k := if Multiplicative.toAdd k = 0 then 1 else invAutZMod n
  map_one' := by simp
  map_mul' := fun a b => by
    ext x
    simp only [MulAut.mul_apply]
    have h1 : (1 : ZMod 2) + 1 = 0 := by decide
    have ha := zmod2_eq_zero_or_one (Multiplicative.toAdd a)
    have hb := zmod2_eq_zero_or_one (Multiplicative.toAdd b)
    rcases ha with ha0 | ha1 <;> rcases hb with hb0 | hb1 <;>
      simp_all [invAutZMod, toAdd_mul, MulAut.one_apply]

@[simp] private lemma negActionZMod_zero (n : ℕ) :
    negActionZMod n (Multiplicative.ofAdd (0 : ZMod 2)) = 1 := by simp [negActionZMod]

@[simp] private lemma negActionZMod_one (n : ℕ) :
    negActionZMod n (Multiplicative.ofAdd (1 : ZMod 2)) = invAutZMod n := by
  simp [negActionZMod]

/-- Map from DihedralGroup n to the semidirect product.
    r i ↦ ⟨i, 0⟩,  sr i ↦ ⟨-i, 1⟩ -/
private noncomputable def dihedralToSemidirectMap (n : ℕ) [NeZero n] :
    DihedralGroup n → Multiplicative (ZMod n) ⋊[negActionZMod n] Multiplicative (ZMod 2)
...
/-- The map from DihedralGroup n to the semidirect product is a group homomorphism. -/
private noncomputable def dihedralToSemidirectHom (n : ℕ) [NeZero n] :
    DihedralGroup n →* Multiplicative (ZMod n) ⋊[negActionZMod n] Multiplicative (ZMod 2) where
  toFun := dihedralToSemidirectMap n
  map_one' := rfl
  map_mul' := fun x y => by
    cases x with
    | r i =>
      cases y with
      | r j =>
        simp only [DihedralGroup.r_mul_r, dihedralToSemidirectMap, SemidirectProduct.mul_def,
          negActionZMod_zero, MulAut.one_apply, ← ofAdd_add, add_zero]
      | sr j =>
        simp only [DihedralGroup.r_mul_sr, dihedralToSemidirectMap, SemidirectProduct.mul_def,
          negActionZMod_zero, MulAut.one_apply, ← ofAdd_add, zero_add, neg_sub]
        ring_nf
    | sr i =>
      cases y with
      | r j =>
        simp only [DihedralGroup.sr_mul_r, dihedralToSemidirectMap, SemidirectProduct.mul_def,
          negActionZMod_one, invAutZMod_apply, toAdd_ofAdd, ← ofAdd_add, add_zero, neg_add]
      | sr j =>
        simp only [DihedralGroup.sr_mul_sr, dihedralToSemidirectMap, SemidirectProduct.mul_def,
          negActionZMod_one, invAutZMod_apply, toAdd_ofAdd, neg_neg, ← ofAdd_add]
        have h : (1 : ZMod 2) + 1 = 0 := by decide
        simp only [h, sub_eq_add_neg, add_comm j (-i)]

private lemma dihedralToSemidirectHom_injective (n : ℕ) [NeZero n] :
    Function.Injective (dihedralToSemidirectHom n) := by
  intro x y hxy
  cases x with
  | r i =>
    cases y with
    | r j =>
      simp only [dihedralToSemidirectHom, MonoidHom.coe_mk, OneHom.coe_mk,
        dihedralToSemidirectMap] at hxy
      have h := SemidirectProduct.ext_iff.mp hxy
      have h1 := h.1
      have h1' := Equiv.injective Multiplicative.ofAdd h1
      exact congrArg DihedralGroup.r h1'
    | sr j =>
      simp only [dihedralToSemidirectHom, MonoidHom.coe_mk, OneHom.coe_mk,
        dihedralToSemidirectMap] at hxy
      have h := SemidirectProduct.ext_iff.mp hxy
      have h2 := h.2
      have h2' := Equiv.injective Multiplicative.ofAdd h2
      exact absurd h2' (by decide : ¬(0 : ZMod 2) = 1)
  | sr i =>
    cases y with
    | r j =>
      simp only [dihedralToSemidirectHom, MonoidHom.coe_mk, OneHom.coe_mk,
      ...
      have h2 := h.2
      have h2' := Equiv.injective Multiplicative.ofAdd h2
      exact absurd h2' (by decide : ¬(1 : ZMod 2) = 0)
    | sr j =>
      simp only [dihedralToSemidirectHom, MonoidHom.coe_mk, OneHom.coe_mk,
        dihedralToSemidirectMap] at hxy
      have h := SemidirectProduct.ext_iff.mp hxy
      have h1 := h.1
      have h1' := Equiv.injective Multiplicative.ofAdd h1
      have h1'' : i = j := neg_inj.mp h1'
      exact congrArg DihedralGroup.sr h1''

private lemma dihedralToSemidirectHom_surjective (n : ℕ) [NeZero n] :
    Function.Surjective (dihedralToSemidirectHom n) := by
  intro ⟨a, b⟩
  rcases zmod2_eq_zero_or_one (Multiplicative.toAdd b) with hb0 | hb1
  · use DihedralGroup.r (Multiplicative.toAdd a)
    simp only [dihedralToSemidirectHom, MonoidHom.coe_mk, OneHom.coe_mk, dihedralToSemidirectMap]
    ext
    · simp only [ofAdd_toAdd]
    · simp only [toAdd_ofAdd, hb0]
  · use DihedralGroup.sr (-(Multiplicative.toAdd a))
    simp only [dihedralToSemidirectHom, MonoidHom.coe_mk, OneHom.coe_mk, dihedralToSemidirectMap]
    ext
    · simp only [neg_neg, ofAdd_toAdd]
    · simp only [toAdd_ofAdd, hb1]

/-- DihedralGroup n is isomorphic to the semidirect product (ZMod n) ⋊ (ZMod 2). -/
private noncomputable def dihedralEquivSemidirect (n : ℕ) [NeZero n] :
    DihedralGroup n ≃* Multiplicative (ZMod n) ⋊[negActionZMod n] Multiplicative (ZMod 2) :=
  MulEquiv.ofBijective (dihedralToSemidirectHom n)
    ⟨dihedralToSemidirectHom_injective n, dihedralToSemidirectHom_surjective n⟩
...
lemma DihedralPointGroup.semidirectProduct (n : ℕ) [NeZero n] :
  ...
  use negActionZMod n
  -- We have: DihedralPointGroup n ≃* DihedralGroup n ≃* (ZMod n) ⋊ (ZMod 2)
  obtain ⟨e1⟩ := DihedralPointGroup.equivDihedralGroup n
  exact ⟨e1.trans (dihedralEquivSemidirect n)⟩
...
lemma CyclicPointGroup.eq_dihedral_inter_SO2 (n : ℕ) [NeZero n] :
  ...
  · intro ⟨hD, hSO⟩
    ...
    | sr i =>
      ...
      simp only [SpecialOrthogonalGroup2] at hSO
